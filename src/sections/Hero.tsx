...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Desktop: centered text + button */}
      <div
        className="absolute inset-0 hidden sm:flex items-end justify-center pb-24 px-5"
        style={{ zIndex: 3 }}
      >
      ...
      >
        <div className="flex flex-col items-center gap-3">
          ...
          {/* Button: text centered, icon pushed to far right */}
          <a
            href="#locations"
            className="flex items-center relative rounded-full font-bold transition-all hover:brightness-110 w-full"
            style={{
              background: '#ffffff',
              color: '#0a0a0a',
          ...
          >
            {/* Invisible spacer to balance the icon, keeping text centered */}
            <span
              className="shrink-0 invisible"
              style={{ width: 44, height: 44 }}
            />
            {/* Centered text */}
            <span className="flex-1 text-center">搜尋聚會地點</span>
            {/* Icon at far right */}
            <span
